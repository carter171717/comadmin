    ...
    table.on('tool(cardList)', function(obj){
        ...
        if(obj.event === 'edit'){
            var editIndex = layer.open({
                title : "编辑卡信息",
                type : 2,
                content : "/admin/creditCard/edit?id="+data.id,
        ...
        if(obj.event === "del"){
            layer.confirm("你确定要删除这张卡么？",{btn:['是的,我确定','我再想想']},
                function(){
                    $.post("/admin/creditCard/delete",{"id":data.id},function (res){
                        if(res.success){
                            layer.msg("删除成功",{time: 1000},function(){
                                table.reload('cardTable', t);
                            });
                        }else{
                            layer.msg(res.message);
                        }
            ...
            if(data.length > 0){
                ...
                layer.confirm("你确定要删除这些卡片么？",{btn:['是的,我确定','我再想想']},
                    function(){
                        ...
                        $.ajax({
                            type:"POST",
                            url:"/admin/creditCard/deleteSome",
                            dataType:"json",
