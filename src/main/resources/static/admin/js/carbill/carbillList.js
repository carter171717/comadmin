layui.use(['layer','form','table'], function() {
    ...
    t = {
        ...
        cols: [[
            {type:'checkbox'},
            {field:'billName', title: '消费名称', width:'10%'},
            {field:'billNum',  title: '消费金额',    width:'10%'},
            {field:'address',     title: '消费地点',    width:'15%' },
            {field:'billDay',     title: '消费日期',    width:'10%' },
            {field:'billType',    title: '消费类型',width:'10%'},
            {field:'payChannel',       title: '支付方式',    width:'10%'},
            {field:'remark', title: '备注', width:'15%'},
            {title: '操作',fixed: 'right', align: 'center', toolbar: '#carbillBar'}
        ]]
    };
    table.render(t);

    //监听工具条
    table.on('tool(carbillList)', function(obj){
        var data = obj.data;
        ...
        if(obj.event === 'edit'){
            var editIndex = layer.open({
                title : "编辑消费信息",
                type : 2,
                content : "/admin/carbill/edit?id="+data.id,
        ...
        if(obj.event === "del"){
            layer.confirm("你确定要删除这条记录么？",{btn:['是的,我确定','我再想想']},
                function(){
                    $.post("/admin/carbill/delete",{"id":data.id},function (res){
                        if(res.success){
